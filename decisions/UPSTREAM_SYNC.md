...
- **影响**: `pnpm run typecheck` (`vue-tsc --noEmit`) 退出码 2；`pnpm run build` (`vue-tsc -b` 严格模式) **同样阻塞生产构建**（`dist/` 无法产出）
- **处置**: 2026-04-12 采取 ADR-001 组合方案 C：fork 内打最小 patch (`src/api/client.ts:95` 带 `// api-station:` marker) 解锁本地冒烟 + 生产构建；同时向 upstream 提交 PR（草稿见 `decisions/upstream-pr-client-ts-cast.md`）；upstream 合并后移除 fork patch
