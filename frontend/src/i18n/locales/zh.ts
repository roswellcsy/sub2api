export default {
  ...
  admin: {
    ...
    settings: {
      ...
      apiStation: {
        fingerprintBilling: {
          title: 'API Station — 指纹与计费',
          description: 'Claude Code 指纹算法和 billing header 配置',
          billingEntrypoint: '计费入口',
          billingEntrypointHint: 'billing header 中 cc_entrypoint 字段的值',
          billingEntrypointPlaceholder: 'cli',
          fingerprintSalt: '指纹盐值',
          fingerprintSaltHint: '消息指纹 SHA256 计算使用的盐值',
          fingerprintSaltPlaceholder: '59cf53e54c78',
          cliVersion: 'CLI 版本覆盖',
          cliVersionHint: '覆盖 billing header 中的 cc_version。留空自动检测。',
          cliVersionPlaceholder: '1.0.33',
          latestKnownCliVersion: '最新已知 CLI 版本',
          latestKnownCliVersionHint: '由版本漂移监控自动检测，也可手动设置。',
          latestKnownCliVersionPlaceholder: '自动检测',
        },
        sessionCooldown: {
          title: 'API Station — 会话与冷却',
          description: '会话 ID 伪装 TTL 和分级冷却退避配置',
          sessionTtlMin: '会话 TTL 最小值（分钟）',
          sessionTtlMinHint: '伪装会话 ID 的最小生存时间',
          sessionTtlMax: '会话 TTL 最大值（分钟）',
        ...
        betaHeaders: {
          ...
          description: '按账号类型和模型配置 anthropic-beta header。留空使用内置默认值。',
          oauth: 'OAuth（非 Haiku）',
          oauthHint: 'OAuth 非 Haiku 模型的逗号分隔 beta 标志',
          oauthHaiku: 'OAuth（Haiku）',
          oauthHaikuHint: 'OAuth Haiku 模型的逗号分隔 beta 标志',
          apikey: 'API Key（非 Haiku）',
          apikeyHint: 'API Key 非 Haiku 模型的逗号分隔 beta 标志',
          apikeyHaiku: 'API Key（Haiku）',
          apikeyHaikuHint: 'API Key Haiku 模型的逗号分隔 beta 标志',
        },
        monitoring: {
          title: 'API Station — 监控与敏感词',
          description: '告警配置、敏感词混淆和监控间隔',
          sensitiveWordsEnabled: '敏感词混淆',
          sensitiveWordsEnabledHint: '在转发前向敏感词中插入零宽字符',
          sensitiveWordsList: '敏感词列表（JSON）',
          sensitiveWordsListHint: 'JSON 数组格式的待混淆词语。留空使用内置默认列表。',
          sensitiveWordsListPlaceholder: '["词语1", "词语2"]',
          feishuWebhookUrl: '飞书 Webhook URL',
          feishuWebhookUrlHint: '封号预警和版本漂移通知的 Webhook 地址',
          feishuWebhookUrlPlaceholder: 'https://open.feishu.cn/open-apis/bot/v2/hook/xxx',
          monitorCheckInterval: '监控检查间隔（秒）',
          monitorCheckIntervalHint: '后台监控服务执行检查的间隔时间',
          banAlertThreshold: '封号预警阈值',
          banAlertThresholdHint: '连续 401/403 失败多少次触发封号预警',
        },
      },
      site: {
        title: '站点设置',
